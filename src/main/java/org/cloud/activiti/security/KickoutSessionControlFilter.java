...
import org.cloud.activiti.redis.RedisManager;
import org.cloud.activiti.redis.SerializeUtils;
...
//        }
        ...
        // 初始化用户的队列放到缓存里
        byte [] data = getRedisKickoutKey(username).getBytes();
        logger.error("username; " + username);
        Deque<Serializable> deque = (Deque<Serializable>) SerializeUtils.deserialize(redisManager.get(data));
        ...
        redisManager.set(getRedisKickoutKey(username).getBytes(), SerializeUtils.serialize(deque), 0);
